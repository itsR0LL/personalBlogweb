...
export default function CloudPlayer() {
  ...
  if (isLoading) {
    return (
      <div className="h-full w-full rounded-2xl sm:rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-4 sm:p-6 flex flex-col items-center justify-center transition-colors duration-700">
  ...
  if (playlist.length === 0 || !currentSong) {
    return (
      <div className="h-full w-full rounded-2xl sm:rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-4 sm:p-6 flex flex-col items-center justify-center transition-all duration-700">
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 sm:gap-5 relative z-10 mb-4 sm:mb-6 mt-1 sm:mt-2 min-w-0">
          ...
          <div className="flex-col overflow-hidden w-full min-w-0">
            ...
            <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white truncate drop-shadow-sm transition-colors duration-700">{currentSong.title}</h3>
            <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 font-medium truncate drop-shadow-sm transition-colors duration-700">{currentSong.artist}</p>
          </div>
        </div>

        <div className="relative z-10 mb-2 h-5 sm:h-6 overflow-hidden">
        ...
        <div className="relative z-10 mt-auto">
          {/* 🌟 核心拦截：把进度条的点击也拦住 */}
          <div
             className="flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-slate-600 dark:text-slate-300 font-bold mb-3 transition-colors duration-700"
