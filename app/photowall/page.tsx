...
export default function PhotoWallPage() {
  ...
  useGSAP(
    () => {
      ...
      const cards = gsap.utils.toArray<HTMLElement>("[data-photo-card]", root);
      ...
      const hoverCards = gsap.utils.toArray<HTMLElement>("[data-photo-hover]", root);
      const cleanups = reduceMotion
        ? []
        : hoverCards.map((card) => {
            const enter = () => {
              gsap.to(card, {
    ...
    { scope: galleryRef, dependencies: [currentAlbum?.id ?? "albums", activeQuery], revertOnUpdate: true }
  );
  ...
  return (
    <div className="min-h-screen relative pb-32">
      ...
      <PageTransition>
        <div ref={galleryRef} className="w-full max-w-7xl mx-auto mt-28 px-4 sm:px-10 relative z-10">

          {!currentAlbum && (
            <div className="animate-fade-in-up">
              <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
                <div>
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-widest mb-2 transition-colors duration-700">光影画廊</h1>
                  <p className="text-slate-600 dark:text-slate-400 font-medium tracking-wider transition-colors duration-700">Snapshots from work, travel, and the spaces between releases</p>
                </div>
              ...
              <div className={`transition-opacity duration-300 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
                    ...
                    <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-6 space-y-6">
                      {matchedPhotos.map((photo, index) => (
                        <div
                          ...
                          className="break-inside-avoid relative group rounded-2xl overflow-hidden cursor-zoom-in shadow-lg bg-white/20 dark:bg-slate-800/20 border border-white/30 dark:border-white/10 transition-transform duration-700 hover:scale-[1.01] hover:shadow-2xl hover:shadow-indigo-500/20"
                        >
                          <img src={photo.url} alt={photo.caption} className="w-full h-auto object-cover transform transition-transform duration-[1000ms] group-hover:scale-[1.035]" loading="lazy" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-20 mt-10">
                  {matchedAlbums.map((album) => (
                    ...
                    >
                      <div className="relative w-full max-w-[280px] sm:w-[85%] sm:max-w-none aspect-[4/3] mb-6 sm:mb-8">
                        <div data-photo-layer className="hidden sm:block absolute inset-0 bg-slate-300 dark:bg-slate-700 rounded-[4px] shadow-md transform rotate-6 translate-x-4 translate-y-2 group-hover:rotate-10 group-hover:translate-x-6 transition-transform duration-700 border-[6px] border-white dark:border-slate-200 overflow-hidden opacity-60">
                           {album.photos[2] && <img src={album.photos[2].url} className="w-full h-full object-cover grayscale blur-[2px]" alt="" />}
                        </div>
                        <div data-photo-layer className="hidden sm:block absolute inset-0 bg-slate-200 dark:bg-slate-600 rounded-[4px] shadow-lg transform -rotate-3 -translate-x-2 -translate-y-1 group-hover:-rotate-5 group-hover:-translate-x-4 transition-transform duration-700 border-[6px] border-white dark:border-slate-200 overflow-hidden opacity-80 z-10">
                           {album.photos[1] && <img src={album.photos[1].url} className="w-full h-full object-cover grayscale-[50%]" alt="" />}
                        </div>
                        <div data-photo-layer className="absolute inset-0 bg-white dark:bg-slate-200 rounded-[4px] shadow-2xl border-[6px] border-white dark:border-slate-200 overflow-hidden z-20 transform group-hover:-translate-y-1 group-hover:scale-[1.03] transition-transform duration-700 relative">
                          <img src={album.cover} alt={album.title} className="w-full h-full object-cover transition-transform duration-[1200ms] group-hover:scale-[1.035]" />
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex flex-col justify-end p-5">
                            <span className="text-white font-bold text-lg drop-shadow-md translate-y-2 group-hover:translate-y-0 transition-transform duration-700">{album.photos.length} 张照片</span>
                            <span className="text-indigo-300 font-medium text-xs mt-1 drop-shadow-md translate-y-2 group-hover:translate-y-0 transition-transform duration-700 delay-75">Click to Open</span>
                          </div>
                        </div>
                      </div>
            ...
            <div className="animate-fade-in-up">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4 border-b border-slate-300/50 dark:border-slate-700/50 pb-6">
                <div>
                  ...
                  <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-wider mb-2">{currentAlbum.title}</h1>
              ...
              <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-6 space-y-6">
                {currentAlbum.photos.map((photo, index) => (
                  <div
                    ...
                    key={`${photo.url}-${index}`}
                    onClick={() => setSelectedImage(photo)}
                    className="break-inside-avoid relative group rounded-2xl overflow-hidden cursor-zoom-in shadow-lg bg-white/20 dark:bg-slate-800/20 border border-white/30 dark:border-white/10 transition-transform duration-700 hover:scale-[1.01] hover:shadow-2xl hover:shadow-indigo-500/20 animate-fade-in-up"
                    style={{ animationDelay: `${index * 50}ms` }}
                  >
                    <img src={photo.url} alt={photo.caption || '照片'} className="w-full h-auto object-cover transform transition-transform duration-[1000ms] group-hover:scale-[1.035]" loading="lazy" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-5">
                      {photo.caption && (
      ...
      {selectedImage && (
        ...
        >
          ...
          <img
            ...
            className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl animate-lightbox-image"
      ...
      <style jsx global>{`
        ...
        .animate-fade-in-up { animation: fadeInUp 0.78s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .animate-fade-in { animation: fadeIn 0.42s ease-out forwards; }
        @keyframes lightboxImage {
          from { opacity: 0; transform: translateY(10px) scale(0.985); }
          to { opacity: 1; transform: translateY(0) scale(1); }
        }
        .animate-lightbox-image { animation: lightboxImage 0.58s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
      `}</style>
    </div>
  );
}
