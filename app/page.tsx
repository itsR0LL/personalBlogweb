...
export default function Home() {
  ...
  try {
    if (fs.existsSync(chattersDirectory)) {
      ...
      allChatters = chatterFiles.map(fileName => {
        ...
        const { data, content } = matter(fs.readFileSync(fullPath, 'utf8'));
        const rawDate = data.date || '1970-01-01';
        const cover = data.cover || runtimeConfig.defaultPostCover;
  ...
  const top5Chatters = allChatters.length > 0 ? allChatters.slice(0, 5) : [{ slug: 'none', title: '暂无随笔', description: '记录一段思绪...', cover: runtimeConfig.defaultPostCover, date: '', formattedDate: '' }];
