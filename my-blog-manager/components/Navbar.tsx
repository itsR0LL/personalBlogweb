...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: '项目', href: '/projects', Icon: null },
    { name: '归档', href: '/timeline', Icon: null },
    { name: '照片墙', href: '/photowall', Icon: null },
    { name: '音乐', href: '/music', Icon: null },
    { name: '说说', href: '/moments', Icon: null },
    { name: '杂谈', href: '/chatter', Icon: null },
    { name: '草稿箱', href: '/drafts', Icon: FileText },
    { name: '友链', href: '/friends', Icon: null },
  ...
  const handleSyncBlogClick = () => {
    if (!targetBlogPath) {
       const fallback = localStorage.getItem('targetBlogPath') || "E:/Project/personalBlogweb";
          ...
          <div className="flex items-center gap-6">
            <nav className="hidden lg:flex gap-8 text-sm font-bold">
              {navLinks.map((link) => {
                ...
                return (
                  <Link key={link.href} href={link.href} className={`relative py-1 transition-colors flex items-center gap-1.5 ${pathname === link.href ? 'text-indigo-600' : 'text-slate-700 dark:text-slate-200'}`}>
                ...
                {isOpBoxOpen && (
                  ...
                  >
                      ...
                      {operations.length === 0 ? (
                        ...
                        operations.map(op => (
                          <div key={op.id} className="bg-white/50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700 flex justify-between items-center group">
                            ...
                            <button onClick={() => removeOperation(op.id)} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 transition-all text-lg">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      ...
                      <button onClick={handleUpdateLocal} className="py-2.5 rounded-xl bg-indigo-500 text-white text-xs font-black shadow-lg shadow-indigo-500/30 hover:bg-indigo-600 transition-colors flex items-center justify-center gap-1.5">
