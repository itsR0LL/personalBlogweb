...
export default function MusicPage() {
  const {
    ...
    isLoading, togglePlay, nextSong, prevSong, handleSeek, seekToTime,
    playSong, selectSong,
  ...
  const filteredPlaylist = useMemo(() => {
    ...
    return playlist.filter((song: MusicSong) =>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-2.5">
                      <AnimatePresence mode='popLayout'>
                        {filteredPlaylist.map((song) => {
                          const originalIndex = playlist.findIndex((s: MusicSong) => s.id === song.id);
